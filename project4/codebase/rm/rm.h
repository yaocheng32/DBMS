...
public:
  ...
  RC close();
  RC addTuple(const RID &rid);
  RC init(string recname, const vector<Attribute> &attrs, const vector<string> &attributeNames);
  int getNumOfTuples();
  void fetchRecord(void *buffer, vector<int> indexes, vector<Attribute> &attrs, void *formated, short *length);
...
private:
  ...
  RC scanFromFile(const string conditionAttribute, const CompOp compOp,                  
      const void *value, const vector<string> &attributeNames, RM_ScanIterator &rm_ScanIterator,
      vector<Attribute> &attrs, string recname);
