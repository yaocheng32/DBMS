...
RC RM::createTable(const string tableName, const vector<Attribute> &attrs) {
	...
	rc = scanFromFile(CataRecHandle, "TableName", EQ_OP, forscanvalue, attributeNames, rmsi, Cata_attrs);
	assert(rc == success);
	free(forscanvalue);
	if (rc != success) {
...
RC RM::readTuple(const string tableName, const RID &rid, void *data) {
	...
	rc = readTupleFromFile(attrs, recHandle, rid, data);
	if (rc != success) {
		//printf("readTupleFromFile in readTuple error\n");
		return -1;	
	}
...
RC RM::getAttributes(const string tableName, vector<Attribute> &attrs) {
	...
	memcpy(forscanvalue, &tmp, 4);
	memcpy((char*)forscanvalue+4, tableName.c_str(), tmp);
	rc = scanFromFile(CataRecHandle, "TableName", EQ_OP, forscanvalue, Cata_attr_names, rmsi, Cata_attrs);
	free(forscanvalue);
	if (rc != success) {
...
RC RM::scanFromFile(PF_FileHandle recHandle,
	...
	//if (!validcondition) return -1;

	RC rc;
	rc = rm_ScanIterator.init();
	...
	for (unsigned i = 0; i < recHandle.GetNumberOfPages(); i++) {
		...
		for (short j = 0; j < nslot; j++) {
			...
			if (conditionAttribute == "") {
				// return all
				// int len;
				// void *projdata = filterRecord(attributeNames, attrs, (char*)page+offset+2, &len);
				// rm_ScanIterator.addTuple(rid, projdata, len);
				// !!!!!!!!!!!!!!!!!!!!!!!!!!			
				original = getOriginalFormat(all_indexes, attrs, formateddata, &ori_len);
				rm_ScanIterator.addTuple(rid, original, (int)ori_len);
				// don't free original!
			} else {
...
RC RM::scan(const string tableName,
	...
	rc = scanFromFile(recHandle, conditionAttribute, compOp, value, attributeNames, rm_ScanIterator, attrs);
...
RC RM_ScanIterator::addTuple(const RID &rid, void *data, int len) {
...
RC RM_ScanIterator::init() {
	...
	Data_list = new vector<void*>();
	Len_list = new vector<int>();
	current_tuple = 0;
	return 0;
}
...
RC RM_ScanIterator::getNextTuple(RID &rid, void *data) {
	...
	memcpy(data, Data_list->at(current_tuple), Len_list->at(current_tuple));
	current_tuple++;
	return 0;
}
